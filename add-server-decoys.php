...
//main docker if loop
//ADD $WSO2 == 'on',$SAMBA == 'on', $MS17 == 'on' 
    if ($SSH == 'on' || $RDP == 'on' || $TFTP == 'on' || $VNC == 'on' || $MSSQL == 'on' || $TELNET == 'on' || $SMB == 'on' || $FTP == 'on' || $MYSQL == 'on' || $WEB == 'on' || $MODBUS == 'on' || $SNMP == 'on' || $S7COMM == 'on' || $HONEYCOMB == 'on' || $WSO2 == 'on' || $SAMBA == 'on' || $MS17 == 'on')
    {
...
if ($MS17 == 'on')
{
    ...
    if (!empty($ipa))
    {
        exec("sudo /usr/bin/docker run -d --name $decoyname\"_MS17\" -p 445:445 asg1612/ms17-010", $outputMS17, $resultMS17);
        $dockeripcheck = 0;
        while ($dockeripcheck == 0)
        {
            global $dockeripcheck;
            exec("sudo /usr/bin/docker inspect $decoyname\"_MS17\" | grep -iw \"IPAddress\" | head -1 | awk -F \"\\\"\" '{print $4}'", $outputMS17ip, $resultMS17ip);
            $dockerip = $outputMS17ip[0];
            $dockeripcheck = preg_match('/^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/', $outputMS17ip[0], $out);
        }
        exec("sudo /bin/ip rule add from $dockerip/32 table $new_if_count priority 11", $outputMS17rule, $resultMS17rule);
    }
    exec("sudo /usr/bin/docker inspect $decoyname\"_MS17\" | grep -i \"LogPath\" | awk -F \"\\\"\" '{print $4}' | sed 's/$//g'", $outputMS17log, $resultMS17log);

    //
    $mysqli = db_connect();

    $stmt = $mysqli->prepare("UPDATE decoys set services=CONCAT(services,'MS17; ') where decoyname=?");
    ...
    $stmt2 = $mysqli->prepare("INSERT into decoydetails (decoyname,decoyservicename,decoyinternalip,decoyroutetable,decoylogfile) VALUES(?,'MS17',?,?,?)");

    if (!$stmt2) {
        throw new Exception('Error in preparing statement: ' . $mysqli->error);
    }

    $stmt2->bind_param("ssss", $decoyname, $dockerip, $new_if_count, $outputMS17log[0]);
    ...
    //
    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"MS17\" \"$outputMS17log[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" > /dev/null 2>&1 &", $output, $result);
}
//AD MS17-010

// ADD SAMBA-CVE-2017-7494
if ($SAMBA == 'on')
{
    $dockerip=trim(' ');
    ...
    if (!empty($ipa))
    {
        exec("sudo /usr/bin/docker run -d --name $decoyname\"_SAMBA\" -p 137-139:137-139 -p 445:445 -p 6699:6699 vulnerables/cve-2017-7494", $outputSAMBA, $resultSAMBA);
        $dockeripcheck=0;
        while ($dockeripcheck == 0)
        {
            global $dockeripcheck;
            exec("sudo /usr/bin/docker inspect $decoyname\"_SAMBA\" | grep -iw \"IPAddress\" | head -1 | awk -F \"\\\"\" '{print $4}'", $outputSAMBAip, $resultSAMBAip);
            $dockerip=$outputSAMBAip[0];
            $dockeripcheck=preg_match('/^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/', $outputSAMBAip[0], $out);
        }
        exec("sudo /bin/ip rule add from $dockerip/32 table $new_if_count priority 11", $outputSAMBArule, $resultSAMBArule);
    }
    exec("sudo /usr/bin/docker inspect $decoyname\"_SAMBA\" | grep -i \"LogPath\" | awk -F \"\\\"\" '{print $4}' | sed 's/$//g'", $outputSAMBAlog, $resultSAMBAlog);

    //
    $mysqli = db_connect();

    $stmt = $mysqli->prepare("UPDATE decoys set services=CONCAT(services,'SAMBA; ') where decoyname=?");
    ...
    $stmt2 = $mysqli->prepare("INSERT into decoydetails (decoyname,decoyservicename,decoyinternalip,decoyroutetable,decoylogfile) VALUES(?,'SAMBA',?,?,?)");

    if (!$stmt2) {
        throw new Exception('Error in preparing statement: ' . $mysqli->error);
    }

    $stmt2->bind_param("ssss", $decoyname, $dockerip, $new_if_count, $outputSAMBAlog[0]);
    ...
    //
    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"SAMBA\" \"$outputSAMBAlog[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" > /dev/null 2>&1 &", $output, $result);
}
// ADD SAMBA-CVE-2017-7494
...
if ($WSO2 == 'on')
{
    ...
    if (!empty($ipa))
    {
        exec("sudo /usr/bin/docker run -d --name $decoyname\"_wso2\" -p 8280:8280 -p 8243:8243 -p 9443:9443 wso2/wso2am:3.0.0", $outputwso2, $resultwso2);
        $dockeripcheck=0;
        while ($dockeripcheck == 0)
        {
            global $dockeripcheck;
            exec("sudo /usr/bin/docker inspect $decoyname\"_wso2\" | grep -iw \"IPAddress\" | head -1 | awk -F \"\\\"\" '{print $4}'", $outputwso2ip, $resultwso2ip);
            $dockerip=$outputwso2ip[0];
            $dockeripcheck=preg_match('/^[0-9]+\.[0-9]+\.[0-9]+\.[0-9]+/', $outputwso2ip[0], $out);
        }
        exec("sudo /bin/ip rule add from $dockerip/32 table $new_if_count priority 11", $outputwso2rule, $resultwso2rule);
    }
    exec("sudo /usr/bin/docker inspect $decoyname\"_wso2\" | grep -i \"LogPath\" | awk -F \"\\\"\" '{print $4}' | sed 's/$//g'", $outputwso2log, $resultwso2log);

    //
    $mysqli = db_connect();

    $stmt = $mysqli->prepare("UPDATE decoys set services=CONCAT(services,'WSO2; ') where decoyname=?");
    ...
    $stmt2 = $mysqli->prepare("INSERT into decoydetails (decoyname,decoyservicename,decoyinternalip,decoyroutetable,decoylogfile) VALUES(?,'wso2',?,?,?)");

    if (!$stmt2) {
        throw new Exception('Error in preparing statement: ' . $mysqli->error);
    }

    $stmt2->bind_param("ssss", $decoyname, $dockerip, $new_if_count, $outputwso2log[0]);
    ...
    //
    exec("sudo /usr/bin/nohup /bin/sh /etc/log.sh \"wso2\" \"$outputwso2log[0]\" \"$decoyname\" \"$decoygroup\" \"$ipa\" \"$decoy_type\" > /dev/null 2>&1 &", $output, $result);
}
// ADD WSO2
		  ...
		  <label>
		    ...
		    $('#rdpservercheck').change(function () {
		      ...
		      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;
                  ...
                  <!-- START MS17-010 -->
                  <label>
                      <input type="checkbox" class="flat-red" name="MS17">
                      MS17-010
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;
                  </label>
                  <!-- END MS17-010   -->
                  ...
                  <!-- START SAMBA -->
                  <label>
                      <input type="checkbox" class="flat-red" name="SAMBA">
                      SAMBA-CVE-2017-7494
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;
                  </label>
                  <!-- END SAMBA   -->
                  ...
                  <!-- START WSO2 -->
                  <label>
                      <input type="checkbox" class="flat-red" name="WSO2">
                      WSO2
                      &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp&nbsp;&nbsp;&nbsp;&nbsp;
                  </label>
                  <!-- END WSO2   -->
